...
export const Chat = () => {
    ...
    useEffect(() => {
        ...
        if (contextoInicial) {
            ...
            setTimeout(async () => {
                ...
                setCargando(true);
                try {
                    // timeout 60 segundos — contexto inicial
                    ...
                    if (err.name !== "AbortError") console.error("error enviando contexto inicial:", err);
                } finally {
                    setCargando(false);
                }
            }, 300);
        ...
        try {
            ...
            if (err.name === "AbortError") {
                setMensajes(prev => [...prev, {
                    role: "assistant",
                    content: "GIA tardó demasiado en responder. El servidor puede estar iniciándose, inténtalo de nuevo.",
    ...
    return (
        <div style={{ display: "flex", flexDirection: "column", height: "100%", background: bg }}>
            ...
            {/* topbar */}
            <div style={{ flexShrink: 0, padding: "14px 24px", borderBottom: `1px solid ${borderColor}`, display: "flex", alignItems: "center", justifyContent: "space-between", background: bg }}>
                <span style={{ fontSize: "14px", fontWeight: tituloProyecto ? "500" : "400", color: tituloProyecto ? (isDark ? "#FAF8F6" : "#3C5160") : "#BAB3AE" }}>
                    {tituloProyecto || "Nueva conversación"}
                </span>
                <button
                    onClick={() => { setMensajes([]); setConversacionActiva(null); setTituloProyecto(""); navigate("/chat"); }}
                    style={{ display: "flex", alignItems: "center", gap: "6px", padding: "6px 12px", borderRadius: "8px", background: "transparent", border: `1px solid ${borderColor}`, fontSize: "12px", color: "#BAB3AE", cursor: "pointer" }}
                >
                    ...
                    <div style={{ maxWidth: "720px", margin: "0 auto", padding: "0 24px", display: "flex", flexDirection: "column", gap: "24px" }}>
                        {mensajes.map((msg, index) => (
                            <div key={index} style={{ display: "flex", flexDirection: "column", alignItems: msg.role === "user" ? "flex-end" : "flex-start" }}>
                                <div style={{
                                    ...
                                    background: msg.role === "user" ? "#3C5160" : bgMsg,
                                    ...
                                    {msg.content.startsWith("[imagen:") ? (
                                        <img src={msg.content.replace("[imagen:", "").replace("]", "")} alt="imagen enviada" style={{ maxWidth: "100%", borderRadius: "10px", maxHeight: "280px", objectFit: "cover" }} />
                                    ) : (
                        ...
                        {cargando && (
                            <div style={{ display: "flex", alignItems: "flex-start" }}>
                                <div style={{ padding: "14px 18px", borderRadius: "18px 18px 18px 4px", background: bgMsg, border: `1px solid ${borderColor}`, display: "flex", gap: "5px", alignItems: "center" }}>
                                    {[0, 150, 300].map(delay => (
                                        <div key={delay} style={{ width: "6px", height: "6px", borderRadius: "50%", background: "#BAB3AE", animation: "bounce 1.2s infinite", animationDelay: `${delay}ms` }} />
                                    ))}
                                </div>
                            </div>
                        )}
            ...
            {/* input */}
            <div style={{ flexShrink: 0, padding: "16px 24px 20px", borderTop: `1px solid ${borderColor}`, background: bg }}>
                <div style={{ maxWidth: "720px", margin: "0 auto", display: "flex", flexDirection: "column", gap: "10px" }}>
                    <div style={{ display: "flex", alignItems: "flex-end", gap: "8px", padding: "10px 14px", borderRadius: "14px", border: `1px solid ${borderColor}`, background: isDark ? "#2C323C" : "#ffffff" }}>
                        ...
                        <div style={{ display: "flex", gap: "4px", flexShrink: 0 }}>
                            <button onClick={() => fileInputRef.current?.click()} disabled={subiendoPDF || cargando} title="Subir manual PDF"
                                style={{ width: "32px", height: "32px", borderRadius: "8px", display: "flex", alignItems: "center", justifyContent: "center", background: "transparent", border: "none", cursor: "pointer", color: subiendoPDF ? "#A9895C" : "#BAB3AE", opacity: cargando && !subiendoPDF ? 0.4 : 1 }}>
                                {subiendoPDF ? <div style={{ width: "14px", height: "14px", border: "2px solid #A9895C", borderTopColor: "transparent", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} /> : <Paperclip size={16} strokeWidth={1.5} />}
                            </button>
                            <button onClick={() => imgInputRef.current?.click()} disabled={subiendoImagen || cargando} title="Subir fotografía"
                                style={{ width: "32px", height: "32px", borderRadius: "8px", display: "flex", alignItems: "center", justifyContent: "center", background: "transparent", border: "none", cursor: "pointer", color: subiendoImagen ? "#A9895C" : "#BAB3AE", opacity: cargando && !subiendoImagen ? 0.4 : 1 }}>
                                {subiendoImagen ? <div style={{ width: "14px", height: "14px", border: "2px solid #A9895C", borderTopColor: "transparent", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} /> : <Camera size={16} strokeWidth={1.5} />}
                            </button>
                            <button onClick={iniciarVoz} disabled={cargando} title="Dictado de voz"
                                style={{ width: "32px", height: "32px", borderRadius: "8px", display: "flex", alignItems: "center", justifyContent: "center", background: escuchando ? "rgba(169,137,92,0.15)" : "transparent", border: "none", cursor: "pointer", color: escuchando ? "#A9895C" : "#BAB3AE" }}>
                        ...
                        <textarea
                            ...
                            rows={1}
                            style={{ flex: 1, resize: "none", border: "none", outline: "none", background: "transparent", fontSize: "14px", color: isDark ? "#FAF8F6" : "#3C5160", lineHeight: "1.5", maxHeight: "120px", overflowY: "auto", padding: "4px 0" }}
                        />

                        <button onClick={enviarMensaje} disabled={!input.trim() || cargando}
                            style={{ width: "34px", height: "34px", borderRadius: "9px", display: "flex", alignItems: "center", justifyContent: "center", background: input.trim() && !cargando ? "#3C5160" : (isDark ? "#3A4150" : "#DDD6CE"), border: "none", cursor: input.trim() && !cargando ? "pointer" : "default", transition: "background 0.2s", flexShrink: 0 }}>
